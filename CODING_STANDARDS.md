...
The only deviation is preserving trailing commas compared to the standard guide. If your code does not follow these standards,
the CI will fail. We run this step:

```yml
- name: Format check
  run: dart format --trailing-commas=preserve -o none --set-exit-if-changed .
```