...
class SBBTabBarController {
  ...
  final _navigationController =
      StreamController<SBBTabBarNavigationData>.broadcast();
  final _layoutController = StreamController<SBBTabBarLayoutData>.broadcast();
  final _warningController =
      StreamController<List<SBBTabBarWarningSetting>>.broadcast();

  /// The current navigation data.
  Stream<SBBTabBarNavigationData> get navigationStream =>
  ...
  Stream<List<SBBTabBarWarningSetting>> get warningStream =>
  ...
  late SBBTabBarLayoutData currentLayoutData = SBBTabBarLayoutData(
    0.0,
    tabs.map((_) => Offset(-100.0, 0.0)).toList(),
  );
  ...
  void updateCurveAnimation() {
    curves.mapIndexed((i, p) {
      ...
      final rightProgress =
          (i == currentLayoutData.positions.length - 1)
              ? 0.0
              : tabStates[i + 1];
      ...
      final double rightMidX =
          (i == curves.length - 1) ? 0.0 : curves[i + 1].midX;
  ...
  void onLayout(List<Offset> positions, double height) {
    if (positions.equals(currentLayoutData.positions) &&
